...
type NegativeTargetTypeChip = "all" | "keyword" | "product";
...
export default function AdsNegativeTargetsConsolePage() {
  ...
  const [activeFilters, setActiveFilters] = useState<AdsConsoleNegativeTargetsQuery | null>(null);
  ...
  const targetsQuery = useQuery({
    queryKey: ["ads-console-negative-targets", demoMode, queryParams],
    queryFn: () => fetchAdsConsoleNegativeTargets(queryParams!),
    enabled: (statusQuery.data?.canGatherData === true || demoMode) && filtersApplied && queryParams != null,
    retry: false,
  });

  const negativeTargets = targetsQuery.data?.negativeTargets ?? [];
  const total = targetsQuery.data?.total ?? 0;
  const requiresFilters = targetsQuery.data?.requiresFilters ?? !filtersApplied;

  const allIds = useMemo(() => negativeTargets.map((t) => t.negativeTargetId), [negativeTargets]);
  ...
  function loadDemoData() {
    ...
    toast({ title: "Demo data loaded", description: "Showing sample negative targets for UI preview." });
  }
  ...
  function bulkComingSoon() {
    toast({
      title: "Bulk negative target actions",
      description: "Bulk enable, pause, and archive for negative targets will be available in a future release.",
    });
  }
    ...
    <AdsConsoleLayout>
      {demoMode && (
        <div className="mb-4 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3">
          <p className="text-sm text-teal-800">
            <span className="font-medium">Demo mode</span> — sample negative target rows for UI preview. Add{" "}
        ...
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <p className="text-sm text-slate-700 flex-1 min-w-0">
              Connect Amazon Ads on Marketplaces and select a profile to load live negative target data.
            </p>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {(
          [
            ...
            { id: "keyword" as const, label: "Negative Keyword" },
            { id: "product" as const, label: "Negative Products" },
          ]
        ).map((chip) => {
      ...
      {targetsQuery.isError && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          {targetsQuery.error instanceof Error ? targetsQuery.error.message : "Failed to load negative targets"}
        </div>
      )}
      ...
      {targetsQuery.isLoading && filtersApplied ? (
        ...
        <AdsConsoleTableShell empty emptyMessage="No negative targets match your filters.">
          <div />
        </AdsConsoleTableShell>
      ) : (
        <>
            ...
            <Table className="min-w-[1100px]">
              <TableHeader>
                <TableRow className="bg-slate-50/80 hover:bg-slate-50/80">
                  ...
                  <TableHead className="min-w-[12rem]">Negative Target</TableHead>
              ...
              <TableBody>
                {negativeTargets.map((row) => (
                  <TableRow key={row.negativeTargetId} className="hover:bg-slate-50/50">
                    <TableCell className="sticky left-0 z-10 bg-white">
                      <Checkbox
                        checked={selected.has(row.negativeTargetId)}
                        onCheckedChange={() => toggleOne(row.negativeTargetId)}
                        aria-label={`Select ${row.negativeTarget}`}
                      />
                    </TableCell>
                    <TableCell className="font-medium text-slate-800">{row.negativeTarget}</TableCell>
                    ...
                    <TableCell className="text-slate-600">{formatMatchType(row.matchType)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      <Dialog open={filterOpen} onOpenChange={setFilterOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div>
              <Label className="text-xs text-slate-500">Search (negative target or campaign)</Label>
              <Input
                className="mt-1"
