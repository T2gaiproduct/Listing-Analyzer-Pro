...
function buildDefaultRedirect(): string {
  if (typeof window === "undefined") {
    return `${basePath}/api/amazon/oauth/callback`.replace(/([^:]\/)\/+/g, "$1");
  }
...
function saveSettings(category: string, settings: Record<string, string>): Promise<{ success?: boolean; warning?: string }> {
  return fetch(`${basePath}/api/admin/settings`, {
    ...
    const data = await r.json().catch(() => ({})) as { error?: string; warning?: string; success?: boolean };
...
function SecretInput({
  ...
  placeholder?: string;
  helpWhenSaved?: string;
}) {
  const [show, setShow] = useState(false);
  const savedAndEmpty = hasSavedValue && value === "";
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1">
        ...
        {savedAndEmpty && (
          <span className="text-[11px] font-medium text-green-700 bg-green-50 border border-green-200 rounded px-1.5 py-0.5">
            Saved
          </span>
        )}
      </div>
      {savedAndEmpty && (
        <p className="text-xs text-green-700 mb-2 flex items-start gap-1">
          <CheckCircle2 className="h-3.5 w-3.5 mt-0.5 shrink-0" />
          {helpWhenSaved ?? "Stored securely and hidden after save. Leave blank to keep it, or paste a new value to replace it."}
        </p>
      )}
      <div className="relative">
        <Input
          ...
          className="pr-10"
          placeholder={savedAndEmpty ? "Paste here only to replace saved secret" : placeholder}
        />
...
export default function AdminSettingsAmazon() {
  ...
  const defaultRedirect = buildDefaultRedirect();
  ...
  useEffect(() => {
    ...
    if (!next.amazon_sp_redirect_uri) {
      ...
      next.amazon_sp_redirect_uri = next.amazon_sp_redirect_uri.replace(/([^:]\/)\/+/g, "$1");
    }
  ...
  const saveMut = useMutation({
    ...
    onSuccess: (result) => {
      const savedSecrets = new Set(maskedFields);
      if (form.amazon_sp_client_secret.trim()) savedSecrets.add("amazon_sp_client_secret");
      if (form.amazon_aws_secret_access_key.trim()) savedSecrets.add("amazon_aws_secret_access_key");
      setMaskedFields(savedSecrets);
      setForm((f) => ({
        ...f,
        amazon_sp_client_secret: "",
        amazon_aws_secret_access_key: "",
      }));
      void qc.invalidateQueries({ queryKey: ["admin-settings-amazon"] });
      toast({ title: "Amazon settings saved" });
      if (result.warning) {
        toast({
          title: "AWS keys not saved",
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <SecretInput
            ...
            helpWhenSaved="Your LWA Client Secret is saved in the database. This field is empty on purpose after save — leave blank to keep it."
          />
