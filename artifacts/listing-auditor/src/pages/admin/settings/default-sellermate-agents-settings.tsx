...
export type DefaultSellermateAgentTemplate = {
  slug: string;
  name: string;
  description: string;
  icon: string;
...
async function fetchDefaultAgents(): Promise<DefaultAgentsResponse> {
  const res = await fetch("/api/admin/sellermate/default-agents");
  if (!res.ok) {
...
export function DefaultSellermateAgentsSettings() {
