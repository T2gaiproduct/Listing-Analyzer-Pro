...
| Blank sign-in / `invalid secret key` on `/api/__clerk` | `CLERK_SECRET_KEY` must match `VITE_CLERK_PUBLISHABLE_KEY` (same Clerk instance). Run `bash scripts/verify-clerk-proxy.sh https://sellerlens.io`. **Dev keys (`pk_test_`) cannot use the Clerk proxy** — deploy latest frontend (skips proxy for `pk_test_`). For live traffic use **production** Clerk keys (`pk_live_` / `sk_live_`), set `proxy_url` to `https://sellerlens.io/api/__clerk` on the **production** Clerk instance (`bash scripts/fix-clerk-production-proxy.sh`), then rebuild and restart. |
| Google sign-in redirects to Cloudflare / wrong host (IP-only deploy) | Set `APP_URL=http://YOUR.IP` (no trailing path). **Do not** set `CLOUDFLARE_TUNNEL_PUBLIC_URL` on production. Point Clerk `proxy_url` at your real origin: `bash scripts/fix-clerk-production-proxy.sh http://103.174.102.70` (use your IP). Rebuild frontend **without** `VITE_CLERK_PROXY_URL` unless it matches that same origin. In Clerk Dashboard add allowed origins / redirect URLs for `http://YOUR.IP` and `/sso-callback`. Remove any `*.trycloudflare.com` proxy_url left from dev. |
